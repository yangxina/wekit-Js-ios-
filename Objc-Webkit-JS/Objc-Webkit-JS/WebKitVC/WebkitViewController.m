...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if ([message.name isEqualToString: kListener]) {
        ...
        _barButtonLeft.tintColor = [UIColor redColor];
        _barButtonRight.tintColor = [UIColor redColor];
    } else if ([message.name isEqualToString: kListenerOther]) {
        NSLog(@"JS通过 <ListenerOther> 调用原生传来参数： \n%@", message.body);
        _barButtonLeft.tintColor = [UIColor greenColor];
        _barButtonRight.tintColor = [UIColor greenColor];
    }
}
